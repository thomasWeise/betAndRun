...
public final class FiniteDifference5 implements UnaryFunctor {
  ...
  /**
   * Instantiates a new finite difference 5.
   *
   * @param inFunc
   *          the in func
   */
  ...
  public double getImage(final double inU) {
    return ((this.mFunc.getImage(inU - (2 * this.mEps))
        - this.mFunc.getImage(inU + (2 * this.mEps)))
        + (8 * (this.mFunc.getImage(inU - this.mEps)
            - this.mFunc.getImage(inU + this.mEps))))
        / (12 * this.mEps);
  }
}
