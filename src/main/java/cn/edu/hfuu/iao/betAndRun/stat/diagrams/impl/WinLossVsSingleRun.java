 ...
 */
public class WinLossVsSingleRun extends WinLossDiagram {
   ...
   *          the bins
   */
  public WinLossVsSingleRun(final Experiments experiments,
      final Path destFolder, final String baseName, final Bin[] bins) {
    super(experiments, destFolder,
        StringUtils.combine(baseName, "winLossVsSingle"), //$NON-NLS-1$
        bins);
  }
  ...
  public final void accept(final Experiments experiments,
    ...
    this.m_data.tick(result.getSetup().division,
        Long.compare(result.getResultQualityWithRuntimeOfDecisionMaking(),
  ...
  protected final void writeData(final PrintWriter dest)
      throws IOException {
    dest.println(
        "# wins and losses against the result that we would get if we just let the first run continue for the complete runtime, i.e., do not apply any bet-and-run strategy at all"); //$NON-NLS-1$
    dest.println();
