 ...
 * included)
 */
public class EqualLossVsBestForBudgetDivisionWithoutRuntime
    extends WinLossDiagram {
   ...
   *          the bins
   */
  public EqualLossVsBestForBudgetDivisionWithoutRuntime(
    ...
    super(experiments, destFolder,
        StringUtils.combine(baseName,
            "equalLossVsBestForDivison_withoutTime"), //$NON-NLS-1$
        bins);
  }
  ...
  protected final void writeData(final PrintWriter dest)
      throws IOException {
    dest.println(
        "# equal scores and losses against the best result of any decision maker on the given budget division (by ignoring the runtime for the decision making process)"); //$NON-NLS-1$
    dest.println(
